...
import { PlanCards } from '../../components/feasibility/PlanCards'
    ...
    const run = async () => {
      try {
        ...
        try {
          ...
          const streamedParsed = feasibilityOutputSchema.safeParse(streamedDonePayload)
          if (!streamedParsed.success) {
            throw new Error('SSE ended without done payload.')
          }

          const streamedOutput: FeasibilityOutput = streamedParsed.data
        ...
        if (shouldFallback) {
          ...
          const output = await jsonPost<FeasibilityInput, FeasibilityOutput>(
            '/agents/feasibility',
            payload,
            { signal: controller.signal }
          )
          const parsed = feasibilityOutputSchema.safeParse(output)
    ...
    return (
      <main className="p-6">
        <GuardPanel
          ...
          description="Feasibility 需要先完成 idea seed、方向和路径选择。"
        />
      </main>
    )
  }
